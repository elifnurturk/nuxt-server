          ...
          <template #default="{ row }">
            <div class="flex justify-end items-center">
              ...
              <el-tooltip content="Delete">
                <button
                  @click="confirmAndDeleteKey(row.Id)"
  ...
  methods: {
    async fetchKeyData() {
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch licence data");
        }
    ...
    async deleteKey(id) {
      try {
        const response = await fetch(`/api/licences/${id}`, {
          method: "DELETE",
        });
        if (!response.ok) {
          throw new Error("Failed to delete Key");
        }
        // Remove the deleted Key from the tableData array
        this.tableData = this.tableData.filter((key) => key.Id !== id);
      } catch (error) {
        console.error("Error deleting Key:", error);
      }
    },
    async confirmAndDeleteKey(id) {
